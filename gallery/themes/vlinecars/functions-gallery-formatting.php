...
{
	$next = str_replace('gallery/news', 'news', getNextNewsURL());
	$prev = str_replace('gallery/news', 'news', getPrevNewsURL());
...
{
	$next = str_replace('gallery/news', 'news', getNextNewsPageURL());
	$prev = str_replace('gallery/news', 'news', getPrevNewsPageURL());
...
<? 	if (zp_loggedin())
	{
		echo "<p>";
		$albumEditURL = str_replace(WEBPATH, '', getAlbumLinkURL());
		echo printLink(WEBPATH . '/zp-core/admin-edit.php?page=edit&album=' . urlencode($albumEditURL), gettext("Edit details"), NULL, NULL, NULL);
		echo '</p>';
	}
?>
	</td>
